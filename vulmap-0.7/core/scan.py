...
from payload.OaFanwei import Oafanwei
from payload.OaTongda import OaTongda
from payload.Oayongyou import oayongyou
from payload.OaZhiyuan import OaZhiyuan
from payload.AtlassianCrowd import AtlassianCrowd
...
class Scan():
    ...
    #OA
    def oa_tongda(self, target, gevent_pool):
        poc_oa_tongda =OaTongda(target)
        gevent_pool.append(spawn(poc_oa_tongda.oa_auth_mobi_userLogin_poc))
        gevent_pool.append(spawn(poc_oa_tongda.oa_login_get2017Session_poc))
        gevent_pool.append(spawn(poc_oa_tongda.oa_login_getV11Session_poc))
        gevent_pool.append(spawn(poc_oa_tongda.oa_uploadfile_file_rce_poc))

    def oa_yongyou(self, target, gevent_pool):
        poc_oa_yongyou = oayongyou(target)
        gevent_pool.append(spawn(poc_oa_yongyou.oa_proxy_CNNVD_201610_923_rce_poc))
        gevent_pool.append(spawn(poc_oa_yongyou.oa_test_sql_poc))
        gevent_pool.append(spawn(poc_oa_yongyou.oa_XbrlPersistenceServlet_rce_poc))
        gevent_pool.append(spawn(poc_oa_yongyou.oa_BshServlet_rce_poc))

    def oa_zhiyuan(self, target, gevent_pool):
        poc_oa_zhiyuan = OaZhiyuan(target)
        gevent_pool.append(spawn(poc_oa_zhiyuan.oa_CNVD_2020_6242_webmail_dow_poc))
        gevent_pool.append(spawn(poc_oa_zhiyuan.oa_thirdpartyController_session_getshell_poc))
        gevent_pool.append(spawn(poc_oa_zhiyuan.oa_getSessionList_session_login_poc))
        gevent_pool.append(spawn(poc_oa_zhiyuan.oa_CNVD_2021_01627_thirdpartyController_up_poc))
        gevent_pool.append(spawn(poc_oa_zhiyuan.oa_CNVD_2019_19299_thirdpartyController_up_poc))
        gevent_pool.append(spawn(poc_oa_zhiyuan.oa_test_sql_poc))

    def oa_fanwei(self, target, gevent_pool):
        poc_oa_fanwei=Oafanwei(target)

        gevent_pool.append(spawn(poc_oa_fanwei.oa_BshServlet_RCE_poc))
        gevent_pool.append(spawn(poc_oa_fanwei.oa_E_Cology_WorkflowServiceXml_RCE_poc))
        gevent_pool.append(spawn(poc_oa_fanwei.oa_loginssoo_sql))
        gevent_pool.append(spawn(poc_oa_fanwei.oa_saveYZJFile_fileName_poc))
        gevent_pool.append(spawn(poc_oa_fanwei.oa_WorkflowCenterTreeData_sql_oracle_poc))

    def Atlassian_Crowd(self, target, gevent_pool):
        poc_atlassian_AtlassianCrowd = AtlassianCrowd(target)
        gevent_pool.append(spawn(poc_atlassian_AtlassianCrowd.Atlassian_Confluence_RCE_poc()))
