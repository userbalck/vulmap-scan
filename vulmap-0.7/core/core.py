...
class Core(object):
    ...
    def scan_webapps(webapps_identify, thread_poc, thread_pool, gevent_pool, target):
        ...
        #ximi 新增
        if r"fanwei" in webapps_identify or r"all" in webapps_identify:
            thread_poc.append(thread_pool.submit(scan.oa_fanwei(target, gevent_pool)))
        if r"tongda" in webapps_identify or r"all" in webapps_identify:
            thread_poc.append(thread_pool.submit(scan.oa_tongda(target, gevent_pool)))
        if r"yongyou" in webapps_identify or r"all" in webapps_identify:
            thread_poc.append(thread_pool.submit(scan.oa_yongyou(target, gevent_pool)))
        if r"zhiyuan" in webapps_identify or r"all" in webapps_identify:
            thread_poc.append(thread_pool.submit(scan.oa_zhiyuan(target, gevent_pool)))

        if r"atlassian" in webapps_identify or r"all" in webapps_identify:
            thread_poc.append(thread_pool.submit(scan.Atlassian_Crowd(target, gevent_pool)))
